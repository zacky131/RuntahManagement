package com.resikin.percakapan.ui.messages.adapters;
...
import com.resikin.percakapan.ui.messages.activities.ImageDetailsActivity;
import com.resikin.percakapan.ui.messages.listeners.OnMessageClickListener;
import com.resikin.percakapan.utils.TimeUtils;
import com.resikin.percakapan.utils.image.ImageUtils;
import com.resikin.percakapan.utils.views.TextViewLinkHandler;
...
class SenderViewHolder extends RecyclerView.ViewHolder {
    ...
    SenderViewHolder(View itemView) {
        super(itemView);
        mMessage = (EmojiTextView) itemView.findViewById(com.resikin.percakapan.R.id.message);
        mDate = (TextView) itemView.findViewById(com.resikin.percakapan.R.id.date);
        mTimestamp = (TextView) itemView.findViewById(com.resikin.percakapan.R.id.timestamp);
        mPreview = (ImageView) itemView.findViewById(com.resikin.percakapan.R.id.preview); // Resolve Issue #32
        mBackgroundBubble = itemView.findViewById(com.resikin.percakapan.R.id.message_group);
        mMessageStatus = (ImageView) itemView.findViewById(com.resikin.percakapan.R.id.status);
        mProgressBar = (ProgressBar) itemView.findViewById(com.resikin.percakapan.R.id.progress);   // Resolve Issue #52
    }
    ...
    private void setFilePreview(final Message message) {
        Glide.with(itemView.getContext())
                ...
                .placeholder(com.resikin.percakapan.R.drawable.ic_placeholder_file_recipient_24dp)
                .into(mPreview);
...
//        mMessage.setMovementMethod(LinkMovementMethod.getInstance()); // clickable link support
    ...
    private void setDate(Message previousMessage, Message message, int position) {
        ...
        if (TimeUtils.isDateToday(message.getTimestamp())) {
            mDate.setText(itemView.getContext().getString(com.resikin.percakapan.R.string.today));
        } else {
    ...
    private void setBubble() {
        ...
        Drawable bubble = ImageUtils.changeDrawableColor(itemView.getContext(),
                com.resikin.percakapan.R.color.background_bubble_sender, com.resikin.percakapan.R.drawable.balloon_sender);
    ...
    private void setStatus(long status) {
        ...
        if (status == Message.STATUS_SENDING) {
            drawableStatus = itemView.getContext()
                    .getResources()
                    .getDrawable(com.resikin.percakapan.R.drawable.ic_message_sending_16dp);
            visible = true;
        } else if (status == Message.STATUS_SENT) {
            drawableStatus = itemView.getContext()
                    .getResources()
                    .getDrawable(com.resikin.percakapan.R.drawable.ic_message_sent_16dp);
            visible = true;
        } else if (status == Message.STATUS_RETURN_RECEIPT) {
            drawableStatus = itemView.getContext()
                    .getResources()
                    .getDrawable(com.resikin.percakapan.R.drawable.ic_message_receipt_16dp);
            visible = true;
        } else {
